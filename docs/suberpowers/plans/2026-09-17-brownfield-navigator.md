...
메모리가 하나도 없으면 수집을 건너뛴다. 조직 이름과 대상 레포 경로를 사용자에게 직접 묻고, `templates/`에서 예시 섹션을 지운 최소 조직 프로필(매칭 조건만 있는 `profile.md`)을 제안한다. 메모리가 쌓인 뒤 이 스킬을 다시 호출하면 규칙을 채울 수 있다고 알린다.
...
2. 매칭 조건을 제안한다
   ...
   - 경로 패턴은 `cd "<레포 경로>" && pwd -P` 결과로 만든다. 그 결과가 실제 홈(`cd ~ && pwd -P`)으로 시작할 때만 앞을 `~/`로 줄인다
