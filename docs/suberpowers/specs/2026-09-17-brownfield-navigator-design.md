...
- **경로 매칭:** 대상 디렉터리(심볼릭 링크를 따라간 실제 경로) **또는 그 상위 디렉터리 중 하나**가 패턴과 일치. 비교는 바이트 단위(`LC_ALL=C`)라 `?`와 `[...]`는 한글 한 글자에 맞지 않으므로, README에 한글 자리에는 `*`를 쓰도록 안내한다. 따라서 `~/work/acme/proj`는 `proj` 하위에서 시작한 세션에도 맞고, `~/work/acme/*`는 `acme` 아래의 모든 레포에 맞는다
