...
- **본문**
  ...
  - `## 호출되었을 때` (id 없음)
    ...
    2. 컨텍스트에 병합된 가이드가 이미 있고 머리말의 매칭 근거가 작업 대상과 같으면 추가로 할 일이 없다. 한 줄 suggest 안내만 있거나 다른 레포 기준 가이드면 다음 단계로 간다
    ...
    4. 출력에 따라 진행한다
       - 병합된 가이드: 어느 조직의 가이드를 적용하는지 한 줄로 알리고 적용한다. 이전에 다른 레포 기준 가이드가 있었으면 새 가이드만 따른다. 경고가 있으면 함께 전달
       - suggest 안내: 안내를 전하고 사용자가 원할 때만 `--manual`로 다시 실행
       - 사용자가 요청했는데 가이드가 없음: 매칭되는 프로필이 없다고 알리고 두 가지를 제안한다(코어 규칙만 적용, `/brownfield-navigator:harvest-profile`로 등록). 사용자가 고르기 전에는 규칙을 적용하지 않는다
       - 스스로 호출했는데 가이드가 없음: 알리지 않고 하던 작업을 계속한다
    5. 대화가 압축된 뒤 병합된 가이드가 컨텍스트에 보이지 않으면 같은 명령을 다시 실행한다 (수동으로 적용한 가이드는 도구 결과로만 남아 압축 때 사라지기 때문)
  - 코어 규칙 섹션 (8장)
